...
import { useAgents, useDepartments, type AgentDetail } from "@/lib/hooks";
...
/** The same org -> department -> agent -> model -> tool -> guardrails walk,
 * shared by the first-run `/welcome` route AND Settings -> General's "Guided
 * setup" re-entry point -- there used to be two separate wizards (this one,
 * and the older InstanceSetupWizard), which had drifted: the older one built
 * its own hardcoded provider/model list and wrote an API key straight to a
 * secret with no CredentialPicker, so it could not actually connect a
 * provider the way AgentModelStep's AddProviderWizard-backed flow now can.
 * Kept step id "org" (not "workspace") to match STEP_ORDER's other ids and
 * OnboardingStepId in mascot.tsx -- renaming it is a pure label choice made
 * in STEP_LABEL, not a step-identity change. */
export function OnboardingWizard({
  onStepChange,
}: {
  /** Fired whenever the current step changes, e.g. so a caller embedding
   * this in a themed layout (see welcome.tsx's mascot mood) can react. */
